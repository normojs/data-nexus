...
- PG Passthrough（A08）：idle pool + `tcp_txn`；TLS pin。**simple Query 透传**；**extended 优先客户端原包 P/B/E TCP（`passthrough_client`，单元级）**；回落 backend 重编码（`passthrough_extended`）；否则 `streaming_demote`；multi-Execute hold 仍 Streaming。
