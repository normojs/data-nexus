...
- **A06 逻辑峰值**：`StreamingEncodeStats.peak_window_rows` + Prometheus `gateway_encode_peak_window_rows`（高水位 gauge）；smoke 在 `window_rows=2` 下断言 peak≤2；**非**进程 RSS 峰值 CI。
- 并发：同一会话事务内 stream 未 drain 前不要再发下一条（producer 持有 lease）。
- Portal（A09）：**NDJSON + CSV + JSON** 在 backend `Streaming` 时窗口 yield → HTTP chunk（`x-data-nexus-stream: backend_window`）；multi-row smoke **强制** 三格式 backend_window；JSON 仍输出完整 `AdminPortalQueryResponse` 文档（分片拼装 rows）；**Complete 回退** 三格式均 `x-data-nexus-stream: chunked` 窗口写出（非 backend_window；backend ResultSet 可能已物化）。
...
- A10 prepared：MySQL COM_STMT_EXECUTE → backend **COM_STMT_PREPARE/EXECUTE 绑定**（连接级 stmt 缓存）+ binary 行解码（**DATE/TIME/DATETIME → ISO 文本**）+ **双协议 ISO 字符串参数启发式绑原生时间类型** + **Streaming 窗口**（`QueryParams`/`Execute`）+ **COM_STMT_RESET no-op OK** + handshake **`8.0.36`**；PG Bind 保留 `$n` → `QueryParams` → prepare/bind + **连接级 Statement 缓存** + **Streaming 窗口**（`query_raw` typed 行）+ **Bind binary i32/i64→text IR** + **INT4 绑 i32**；**PG `GatewayCommand::Execute` 与 QueryParams 同走 Streaming**；**Describe 显式 SELECT 列表 → RowDescription**；**`SELECT *` → backend prepare（`DescribeSql`/`RowDescription`）**；**同连接二次 Bind/Execute 不永久抑制 T**（仅 Describe(P) 一次性 suppress；Sync 清 portal flags）；ParameterDescription OID=text(25)；Bind binary/INT2 含 NUL 的 text-format 启发式；**smoke**：双协议 prepared `max_rows=1` + **psycopg3 rebind** + star catalog；非 TCP passthrough。
