...
- A10 prepared：MySQL COM_STMT_EXECUTE → backend **COM_STMT_PREPARE/EXECUTE 绑定**（连接级 stmt 缓存）+ binary 行解码（**DATE/TIME/DATETIME → ISO 文本**）+ **双协议 ISO 字符串参数启发式绑原生时间类型** + **Streaming 窗口**（`QueryParams`/`Execute`）；PG Bind 保留 `$n` → `QueryParams` → prepare/bind + **连接级 Statement 缓存** + **Streaming 窗口**（`query_raw`）；**PG `GatewayCommand::Execute` 与 QueryParams 同走 Streaming**（未知 id / arity fail-closed）；result_format=1 → binary DataRow（含 date/ts/time）；非 TCP passthrough。
