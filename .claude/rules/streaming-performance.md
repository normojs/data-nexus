...
- Portal（A09）：**NDJSON + CSV + JSON** 在 backend `Streaming` 时窗口 yield → HTTP chunk（`x-data-nexus-stream: backend_window`）；multi-row smoke **强制** 三格式 backend_window；JSON 仍输出完整 `AdminPortalQueryResponse` 文档（分片拼装 rows）；**Complete 回退** 三格式均 `x-data-nexus-stream: chunked` 窗口写出（非 backend_window；backend ResultSet 可能已物化）；**跨协议 portal 双向**：MySQL→PG 与 PG→MySQL（`portal_prepare` translation + 列类型映射）；`smoke-security-portal-xproto{,-pg-mysql}` 强制三格式 backend_window（`window_rows=2`）。
