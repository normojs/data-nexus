...
- **A06 逻辑峰值**：`StreamingEncodeStats.peak_window_rows` + Prometheus `gateway_encode_peak_window_rows`；**`peak_window_bytes` + `gateway_encode_peak_window_bytes`**（单窗 encode 载荷高水位；多窗 smoke peak≪total）；smoke 钉 peak_rows≤window；**非**进程/cgroup 精确 1–2 窗字节。粗粒度 `smoke-security-stream-rss`：双协议 + cgroup/proc/ps + 多窗下限。
