...
- A10 prepared：MySQL COM_STMT_EXECUTE → backend **COM_STMT_PREPARE/EXECUTE 绑定**（连接级 stmt 缓存）+ binary 行解码（**DATE/TIME/DATETIME → ISO 文本**）+ **双协议 ISO 字符串参数启发式绑原生时间类型** + **Streaming 窗口**（`QueryParams`/`Execute`）+ **COM_STMT_RESET no-op OK** + handshake **`8.0.36`**；PG Bind 保留 `$n` → `QueryParams` → prepare/bind + **连接级 Statement 缓存** + **Streaming 窗口**（`query_raw` typed 行）+ **Bind binary i32/i64→text IR** + **INT4 绑 i32**；**PG `GatewayCommand::Execute` 与 QueryParams 同走 Streaming**；**Describe 显式 SELECT 列表 → RowDescription**；**`SELECT *` → backend prepare（`DescribeSql`/`RowDescription`）**（portal/statement Describe 后 Execute 抑制二次 T；非 SELECT 仍 NoData）；ParameterDescription OID=text(25)；Bind binary/INT2 含 NUL 的 text-format 启发式；**smoke**：双协议 prepared `max_rows=1` + **psycopg3**（含 star catalog）；非 TCP passthrough。
