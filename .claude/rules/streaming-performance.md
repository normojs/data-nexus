...
- **A06 逻辑峰值**：`StreamingEncodeStats.peak_window_rows` + Prometheus `gateway_encode_peak_window_rows`（高水位 gauge）；smoke 在 `window_rows=2` 下断言 peak≤2；**非**精密 window 字节 CI。粗粒度 `smoke-security-stream-rss`：**双协议**大结果 Streaming；内存采样 **cgroup→proc→ps**；绝对增长 cap 防全量物化（默认 256MiB）；**encode_windows 多窗下限**；逻辑 peak 仍权威。
...
- PG Passthrough（A08）：idle pool（cap + TTL + SELECT 1 探测）+ 事务 `tcp_txn` 原帧中继；`endpoints[].ssl_mode` disable/prefer/require；**`ssl_ca_file` + `ssl_accept_invalid_certs` 默认 false（verify）**；prod 模板 require+CA PEM；validate 拒绝 require+verify 无 CA。Streaming 仍用 pool。**simple Query 透传 smoke**；**passthrough 下 extended text-bind：backend 重编码 Parse/Bind/Execute/Sync TCP 并标 `passthrough_extended`**（`write_wire_relay_opts` 在 `pg_extended_query` 下剥离 backend `1`/`2`/`Z`）；否则 demote Streaming（`streaming_demote`）；**非**客户端原包帧中继。MySQL prefer 与 PG prefer 同语义（可明文回落）。
