...
function sampleTitle(e: AdminAuditEvent): string {
  ...
  return parts.join(' · ') || 'no sample (need sample_enabled + default_audit_level=L2; not L3 full result; API full_result_l3=false)'
}
      ...
      <div class="form-grid">
        ...
        <label>
          ...
          <input
            v-model="rule"
            class="input mono"
            placeholder="deny-secret-tables"
            @keyup.enter="load"
          >
        </label>
        <label>
        ...
        <tbody>
          ...
          >
            ...
            <td class="mono sample">
              ...
              <span
                v-else
                title="no sample (need sample_enabled + default_audit_level=L2; not L3 full-result archive)"
              >—</span>
            </td>
            <td class="mono eid">
              <button
