...
- [ ] **A10** 预处理 / 事务透传矩阵  
  - 已有：MySQL COM_STMT + Streaming + PREPARE 列定义；PG Parse/Bind/Execute + Streaming；Describe 显式 SELECT + `SELECT *` catalog；**扩展协议 Execute 不发 ReadyForQuery**（仅 Sync 发 Z）→ **同连接二次 Execute/psycopg rebind**；协议 smoke + mysql description  
  - 仍欠：非 TCP passthrough；**PortalSuspended**（客户端 Execute max_rows 未完整对接）；复杂 JOIN `*` 依赖 backend prepare  
  - 路径：frontend/backend mysql+pg、`SessionState.pg_extended_query`、`smoke-security-stream.sh`
...
| 预处理语句 | A10：协议 smoke + mysql description + **psycopg 同连接 rebind**；扩展 Execute 不夹带 Z；**PortalSuspended 仍未做**；非 TCP passthrough |
...
1. **A10** PortalSuspended（客户端 Execute max_rows 门户状态）  
