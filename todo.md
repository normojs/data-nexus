...
| **v2.2** | 大数据热路径封顶 + ABAC/样本/Remote PDP | **进行中**（A 轨部分 + F29/B08；F31 延后） |
...
**>>> A10 参数类型细化 或 push origin 或 F31（延后） <<<**

本轮（A08 生产 TLS 钉扎）：

- prod 模板：`ssl_mode=require` + `ssl_ca_file=__DN_*_SSL_CA_FILE__` + `ssl_accept_invalid_certs=false`
- env/render 增加 `DN_MYSQL_SSL_CA_FILE` / `DN_PG_SSL_CA_FILE`
- validate：`require` + `accept_invalid=false` **必须** `ssl_ca_file`（生产钉扎）
- 默认代码仍 `accept_invalid=true`（不破坏现有 smoke/开发）

```bash
cargo test -p gateway_core --lib a08_
./examples/prod/render-config.sh   # after sourcing env.example
./examples/run-smoke-matrix.sh default
```

建议下一刀：

1. **A10** — 参数类型（date 绑定 / 非通用 scalar）  
2. **push origin**（需明确同意；领先 origin）  
3. **F31** — Remote PDP（延后）
