...
- [ ] **A08** PostgreSQL wire 透传 + backend TLS  
  ...
  - 路径：`backend/postgresql` + `backend/mysql` demote、`pg_tcp_relay`、`write_wire_relay_opts` skip-Z、`core_engine` demote label、`smoke-security-passthrough.sh`、`smoke-security-config-validate.sh`、`OBSERVABILITY.md`
...
| PG/MySQL backend TLS | A08：默认 accept_invalid=**false**（verify）；simple Query 透传；**PG text-bind extended 可 rewrite→simple Query TCP（`QUERY_PARAMS` 标 passthrough + wire bytes；extended 内 strip backend Z）**；不可改写（MySQL COM_STMT）标 **`streaming_demote`**；smoke 强制两者；**仍非** Parse/Bind 原包中继 |
