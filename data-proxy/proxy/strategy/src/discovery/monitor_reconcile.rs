...
use crate::{
    config::{MasterHighAvailability, ReadWriteSplittingDynamic},
...
impl MonitorReconcile {
    ...
    pub fn start_monitor_reconcile(
        ...
        tokio::spawn(async move {
            MonitorReconcile::report(tx, cc, rw_endpoint, monitor_response_channel, monitors_len)
                .await;
        });
    ...
    async fn report(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            loop {
                ...
                Self::change_read_write_if_necessary(
                    &master_high_availability,
                    &mut connect_monitor_response,
                    &mut ping_monitor_response,
                    &mut read_only_monitor_response,
                    &mut curr_rw_endpoint,
                );
                Self::change_read_if_necessary(
                    &master_high_availability,
                    &rw_endpoint,
                    &mut connect_monitor_response,
                    &mut ping_monitor_response,
                    &mut replication_lag_monitor_response,
                    &mut curr_rw_endpoint,
                );
                ...
                std::thread::sleep(std::time::Duration::from_millis(
                    master_high_availability.monitor_period,
                ));
            }
        });
    }

    fn change_read_write_if_necessary(
        master_high_availability: &MasterHighAvailability,
                        ...
                        match read_write_ping_status {
                            ...
                            crate::monitors::ping_monitor::PingStatus::PingNotOk => {
                                if curr_rw_endpoint.clone().read.len() > 0 {
                                    ...
                                    Self::change_slave_to_master_by_read_only(
                                        &master_high_availability,
                                        &mut read_only_monitor_response,
                                        &mut curr_rw_endpoint,
                                    )
    ...
    fn change_read_if_necessary(
        ...
        for (read_addr, read_connect_status) in &connect_monitor_response.as_ref().unwrap().read {
            match read_connect_status {
                crate::monitors::connect_monitor::ConnectStatus::Connected => {
                    match ping_monitor_response.clone() {
                        Some(ping_response) => {
                            for (read_ping_addr, read_ping_status) in ping_response.clone().read {
                                match read_ping_status {
                                    crate::monitors::ping_monitor::PingStatus::PingOk => {
                                        Self::change_read_by_lag(
                                            &master_high_availability,
                                            rw_endpoint,
                                            &mut replication_lag_monitor_response,
                                            &mut curr_rw_endpoint,
    ...
    fn change_read_by_lag(
        master_high_availability: &MasterHighAvailability,
        rw_endpoint: &ReadWriteEndpoint,
        replication_lag_monitor_response: &mut &mut Option<ReplicationLagMonitorResponse>,
        curr_rw_endpoint: &mut &mut ReadWriteEndpoint,
    ) {
                        ...
                        if !lag_status.is_latency {
                                ...
                                .find(|r| r.addr.eq(replication_lag_addr))
                            {
                                ...
                                None => {
                                    curr_rw_endpoint
                                        .read
                                        .append(&mut curr_rw_endpoint.read.clone());
                                }
                            }
                            continue;
                        } else {
                            ...
                            curr_rw_endpoint.read.remove(
                                rw_endpoint
                                    .read
                                    .iter()
                                    .position(|r| r.addr.eq(replication_lag_addr))
                                    .unwrap(),
                            );
                        }
                    }
    ...
    fn change_slave_to_master_by_read_only(
        master_high_availability: &MasterHighAvailability,
        read_only_monitor_response: &mut &mut Option<ReadOnlyMonitorResponse>,
        curr_rw_endpoint: &mut &mut ReadWriteEndpoint,
    ) {
