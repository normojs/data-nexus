...
impl ShardingRewrite {
    ...
    fn database_table_strategy(
        ...
        Ok(self.database_table_strategy_iproduct(
            DatabaseTableStrategyPart::DatabaseTable(
                wheres[0].1[0].unwrap() as usize,
                wheres[0].1[1].unwrap() as usize,
            ),
            try_tables,
            fields,
            orders,
    ...
    fn database_strategy(
        ...
        if wheres.is_empty() {
            return Ok(self.database_strategy_iproduct(
                DatabaseTableStrategyPart::All,
                try_tables,
                fields,
                orders,
        ...
        let output = self.database_strategy_iproduct(
            DatabaseTableStrategyPart::Database(wheres[0].1[0].unwrap() as usize),
            try_tables,
            fields,
            orders,
            groups,
    ...
    fn table_strategy(
        ...
        if wheres.is_empty() {
            return Ok(self.table_strategy_iproduct(
                DatabaseTableStrategyPart::All,
                try_tables,
                fields,
                orders,
        ...
        let output = self.table_strategy_iproduct(
            DatabaseTableStrategyPart::Table(wheres[0].1[0].unwrap() as usize),
            try_tables,
            fields,
            orders,
            groups,
    ...
    fn change_order_group(
        ...
        for field in not_exist_order_fields {
            ...
            let order_as_name = format!(
                "{}_{}_{:05}",
                order_as_name,
                ORDER_BY_DERIVED,
                ds_sharding_idx.idx.table.unwrap()
            );
    ...
    fn change_insert_sql_inner<'b>(
        ...
        let results = sqls
            .into_iter()
            .map(
                |((db_idx, table_idx), v)| -> Result<ShardingRewriteResult, ShardingRewriteError> {
    ...
    fn database_table_strategy_iproduct(
        ...
        for t in tables.iter() {
            ...
            let (actual_nodes, db_idx) = match part {
                ...
                | DatabaseTableStrategyPart::DatabaseTable(idx, _) => {
            ...
            for (node_idx, node) in actual_nodes.iter().enumerate() {
                ...
                let sharding_range = match part {
                    ...
                    | DatabaseTableStrategyPart::DatabaseTable(_, idx) => idx..idx + 1,
                ...
                for table_idx in sharding_range {
                    ...
                    let ds_sharding_idx = DataSourceShardingIdx {
                        idx: ShardingIdx {
                            database: Some(db_idx as u64),
                            table: Some(table_idx as u64),
                        },
                        ds: data_source.clone(),
                        column: ShardingColumn {
                            database: sharding_column.0.map(|x| x.to_string()),
                            table: sharding_column.1.map(|x| x.to_string()),
                        },
                    };
    ...
    fn database_strategy_iproduct(
        ...
        for t in tables.iter() {
            ...
            for (node_idx, node) in actual_nodes.iter().enumerate() {
                ...
                let data_source =
                    self.gen_data_source(&t.1, db_idx.unwrap_or_else(|| node_idx)).unwrap();
    ...
    fn table_strategy_iproduct(
        ...
        for t in tables.iter() {
            ...
            let sharding_range = match part {
                DatabaseTableStrategyPart::Table(idx) => idx..idx + 1,
                _ => 0..sharding_count as usize,
            };
    ...
    fn get_agg_field1(fields: &IndexMap<u8, Vec<FieldMeta>>) -> IndexMap<u8, Vec<FieldMetaIdent>> {
