...
4. **Audit L2 samples (B08)**: leave `sample_enabled=false` unless you need bounded post-mask samples; if enabling, set `default_audit_level=L2` (validate enforces). Not full-result capture.
5. **Backend TLS pin (A08)**: default `ssl_accept_invalid_certs=false` (verify). Template sets `ssl_mode=require` + `ssl_ca_file=__DN_*_SSL_CA_FILE__`. Set `DN_MYSQL_SSL_CA_FILE` / `DN_PG_SSL_CA_FILE` to PEM paths. Config validate rejects require+verify without CA. Dev/self-signed must set `ssl_accept_invalid_certs=true` explicitly. Client TLS still terminates at LB.  
