...
| Passthrough always wire | **Simple Query** same-protocol + no result obligations can be `passthrough` (PG TCP frame relay / MySQL wire). **Extended** under `passthrough=true`: PG text-bindable `QUERY_PARAMS`/`EXECUTE` may **rewrite → simple Query TCP** and still label `passthrough` + `passthrough_bytes` (smoke pins `type=QUERY_PARAMS` + `execute_path=passthrough`); **backend ReadyForQuery from the rewritten simple Query is stripped until client Sync** (extended unit correctness; not original Parse/Bind frame relay). If rewrite cannot apply (e.g. MySQL COM_STMT binary bind) → **`streaming_demote`**. |
...
Related smokes: `smoke-security-stream.sh` (streaming + peak≤window + PortalSuspended resume), `smoke-security-passthrough.sh` (simple passthrough + PG text-bind `QUERY_PARAMS`→passthrough + MySQL `EXECUTE`→`streaming_demote`), `smoke-security-portal*.sh` (backend_window vs chunked).
