...
| Passthrough always wire | **Simple Query** same-protocol + no result obligations can be `passthrough` (PG TCP frame relay / MySQL wire). **Extended** under `passthrough=true`: **prefer text-inline rewrite → simple Query wire** (`passthrough`); if rewrite cannot apply → **`streaming_demote`**. **Not** original Parse/Bind/Execute frame relay. |
