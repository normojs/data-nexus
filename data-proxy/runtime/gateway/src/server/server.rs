...
pub struct LegacyMySqlCommandService<T, C> {
    endpoints: Vec<EndpointConfig>,
    _phat: PhantomData<(T, C)>,
}

impl<T, C> LegacyMySqlCommandService<T, C> {
    pub fn new() -> Self {
        Self { endpoints: Vec::new(), _phat: PhantomData }
    }
}

impl<T, C> Default for LegacyMySqlCommandService<T, C> {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl<'a, T, C> MySqlCommandService<T, C> for LegacyMySqlCommandService<T, C>
where
    T: AsyncRead + AsyncWrite + Unpin + Send,
    C: Decoder<Item = BytesMut>
        + Encoder<PacketSend<Box<[u8]>>, Error = ProtocolError>
