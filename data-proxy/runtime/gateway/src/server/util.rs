...
pub fn filter_avg_column(
    change: &AvgChange,
    column_info: &ColumnInfo,
    is_added: bool,
) -> (Vec<u8>, Option<ColumnInfo>) {
    ...
    if &column_info.column_name == &change.count_field
        || &column_info.column_name == &change.sum_field
    {
        if !is_added {
...
pub fn get_avg_change(changes: &IndexMap<u8, Vec<ChangeTargetMeta>>) -> Option<&AvgChange> {
    match changes.get(&1) {
        Some(changes) => changes.iter().find_map(|meta| {
