...
mod test {
    ...
    fn test_dml_stmt() {
        let inputs = vec![
            ...
            "SHOW COLUMNS FROM `t_order`;",
            "SHOW COLUMNS FROM base_sys_log FROM `panda-admin`;",
            "SHOW COLUMNS FROM `panda-admin`.`base_sys_log2`",
            "SHOW COLUMNS FROM `panda-admin`.`base_sys_log` FROM `panda-admin`;",
            "SHOW COLUMNS FROM `panda-admin`.`base_sys_log` FROM `panda-admin` where 1=1;",
    ...
    fn parser(inputs: Vec<&str>) {
        ...
        for input in inputs {
            ...
            match res {
                Err(e) => {
                    println!("error sql={:?} {:?}", input, e);
                    assert!(isOk);
                }
                Ok(_stmt) => {
                    // println!("success sql={:?} {:#?}", input, _stmt)
                    // println!("success sql={:?}", input)
                }
            }
        }
    }
