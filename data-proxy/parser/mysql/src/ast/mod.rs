...
// limitations under the License. 2111
...
pub enum SqlStmt {
    ...
    ShowTableStatusStmt(Box<ShowTableStatusStmt>),
...
impl SqlStmt {
    pub fn format(&self) -> String {
        match self {
            Self::SelectStmt(stmt) => stmt.format(),

            Self::InsertStmt(stmt) => stmt.format(),

            Self::UpdateStmt(stmt) => stmt.format(),

            Self::DeleteStmt(stmt) => stmt.format(),
