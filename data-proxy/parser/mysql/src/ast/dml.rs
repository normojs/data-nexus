...
impl SelectStmt {
    pub fn format(&self) -> String {
        match self {
            Self::Query(val) => val.format(),

            Self::SubQuery(val) => val.format(),

            Self::With(val) => val.format(),
...
impl SingleTable {
    pub fn format(&self) -> String {
        let mut single = vec![self.table_name.format()];
...
pub struct ShowTableStatusStmt {
    pub span: Span,
    // pub opt_show_cmd_type: Option<ShowCmdType>, // 废弃
    // pub opt_db: Option<ShowTableDb>,
    pub opt_wild_or_where: Option<WildOrWhere>,
}
#[derive(Debug, Clone)]
