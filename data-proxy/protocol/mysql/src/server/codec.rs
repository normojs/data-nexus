...
impl PacketCodec {
    ...
    /// 在给定的字节数组buf的offset位置处设置一个序列号，并将序列号加1
    #[inline]
    fn set_seq_id(&mut self, buf: &mut [u8], offset: usize) {
        unsafe {
            // 具体实现上，函数通过as_mut_ptr()获取buf的原始指针，然后通过add()方法计算出要设置序列号的位置。
            // 接着，使用解引用加偏移的方式*data_ptr.add(3)来访问该位置，并将序列号赋值给它。最后，通过wrapping_add(1)将序列号加1
...
impl Decoder for PacketCodec {
    ...
    fn decode(&mut self, src: &mut BytesMut) -> Result<Option<Self::Item>, Self::Error> {
        ...
        if 4 + length > src.len() {
            // src中表示长度的数据为4个字节，所以判断 payload + 4 > src.len()
            // 表示不完整
            return Ok(None);
        }
        ...
        if length < MAX_PAYLOAD_LEN {
            // self.buf.split()清空buf，但保留buf的容量。并返回buf
...
mod test {
    ...
    use crate::{
        ...
        server::{auth::ServerHandshakeCodec, codec::PacketSend},
    };
