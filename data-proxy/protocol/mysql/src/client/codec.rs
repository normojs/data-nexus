...
use crate::{
    client::resultset::DecodeResultsetState,
    err::ProtocolError,
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        match codec.poll_next(cx) {
            Poll::Ready(Some(Ok(data))) => {
                if next_state == DecodeResultsetState::Row
                    || next_state == DecodeResultsetState::RowBinary
                {
                    if is_eof(&data.0) {
                        return Poll::Ready(None);
                    }
...
impl<'a, T: AsRef<[u8]> + Default + Clone + From<bytes::BytesMut>> Stream
    for QueryResultStream<'a, T>
{
