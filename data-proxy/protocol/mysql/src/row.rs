...
impl<T: AsRef<[u8]> + Default> Clone for RowDataText<T> {
    fn clone(&self) -> Self {
        RowDataText { common: self.common.clone(), buf: T::default() }
    }
}
...
pub fn decode_with_name<'e, T, V>(
    row_data: &mut RowDataTyp<T>,
    name: &str,
    is_binary: bool,
) -> Result<Option<V>, Box<dyn std::error::Error + Send + Sync + 'e>>
where
    T: AsRef<[u8]> + Default,
