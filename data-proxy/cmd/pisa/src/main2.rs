...
fn main() {
    ...
    tracing_subscriber::fmt()
        ...
        .init();
    info!("uni-proxy start: {}", config.get_version());
    ...
    // 3、停止
    // 4、重启
    let factory2: &'static PisaHttpServerFactory = &PisaHttpServerFactory::new(config.clone(), MetricsManager::new());
    let http_server = factory2.build_http_server(HttpServerKind::Axum);
    match config.proxy.clone() {
        Some(_) => {
            ...
            build_runtime().block_on(async move {
                ...
                servers.push(NodeInstance::new("httpserver-001".to_string(), tokio::spawn(new_http_server(http_server))));
